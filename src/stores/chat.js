...
import { generateRoomKey, sealRoomKey, getRoomKey, encryptMessage, decryptMessage, generateKeyPair, hasLocalKeys } from '@/api/crypto';
export const useChatStore = defineStore('chat', () => {
    ...
    async function loadRooms() {
        ...
        rooms.value = (data || []).map((r) => r.rooms).filter(Boolean);
    }
    async function createGroup(title, memberIds) {
        ...
        await supabase.from('chat_room_members').insert(members.map((uid) => ({ room_id: roomId, user_id: uid, role: uid === myId.value ? 'owner' : 'member' })));
    ...
    async function subscribe(roomId) {
        ...
        channel = supabase
            ...
            let text = '(无法解密)';
            try {
                text = await decryptMessage(m.ciphertext, m.nonce, roomKey);
            }
            catch { }
            messages.value.push({
    ...
    async function send(text) {
        if (!activeRoom.value || !myId.value || !text.trim())
            return;
