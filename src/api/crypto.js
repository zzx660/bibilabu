...
export async function sealRoomKey(roomKeyB64, memberPubKeyB64) {
    const s = await ensure();
    const sealed = s.crypto_box_seal(new TextEncoder().encode(roomKeyB64), unb64(memberPubKeyB64));
    return b64(sealed);
}
// 用自己公钥+私钥解密拿到 room_key
export async function openSealedRoomKey(sealedB64) {
    const s = await ensure();
...
export async function encryptMessage(plaintext, roomKeyB64) {
    const s = await ensure();
...
// 对称解密
export async function decryptMessage(ciphertextB64, nonceB64, roomKeyB64) {
    const s = await ensure();
    const key = unb64(roomKeyB64);
...
export async function getRoomKey(roomId, sealedB64) {
...
export function clearRoomKeyCache(roomId) {
    if (roomId)
