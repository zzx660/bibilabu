...
function chunk(text: string, size = 600, overlap = 80): string[] {
  const out: string[] = []
  let i = 0
  while (i < text.length) {
  ...
  for (const p of persons) {
    ...
    for (let idx = 0; idx < chunks.length; idx++) {
      ...
      try {
        ...
        if (insErr) console.warn(`写入失败 ${p.name_zh}:`, insErr.message)
      } catch (e: any) {
        console.warn(`embed 失败 ${p.name_zh}:`, e.message)
      }
    }
    done++
    if (done % 20 === 0) console.log(`  ${done}/${persons.length}`)
  }
